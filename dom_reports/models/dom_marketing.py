...
class DomMarketing(models.TransientModel):
    ...
    def set_values(self):
        ...
        ).set_param('domreps_top_message', self.top_message)
        self.env['ir.config_parameter'].sudo(
        ).set_param('domreps_footer_logo_1', self.footer_logo_1)
        self.env['ir.config_parameter'].sudo(
        ).set_param('domreps_footer_logo_2', self.footer_logo_2)
        self.env['ir.config_parameter'].sudo(
        ).set_param('domreps_footer_logo_3', self.footer_logo_3)
        self.env['ir.config_parameter'].sudo(
        ).set_param('domreps_footer_logo_4', self.footer_logo_4)
